...
- [x] 224. Fighter lasers: shots follow one shared schedule (world.js STRIKE_SHOTS for the strike chase, DOG_SHOTS for the background dogfight) — HITS strike the target (impact flash, glowing scorch and spark particles riding on the hull, a small light), MISSES fly on 4 km out into space at the same speed. Sound: the metronomic 'laser_shot' sample is replaced by a synthesized pitch-varied blaster zap on every visible shot + a spark crackle on hits.
- [x] 225. Glow → BLOOM: the bloom pass now has a soft-knee threshold (only HDR > ~1 blooms) and real strength (0.022 → 0.3); the big fake-halo sprites are gone (explosion wash/halos, nozzle halos + engine base glow, bolt-burst halos, dogfight impact halo) — bright HDR cores remain and the bloom makes their glow; every remaining glow sprite's soft halo term ×0.35.
- [x] 226. The launch-run swat: the arm flings out to the upper RIGHT (~45° up, FK-solved, kept near the wind-up pose so the swing path stays clean); no blast any more (burst/fireball/shock/explosion SFX removed) — only a brief hot contact point and a radial fan of sparks along the swing (±35°), cooling and thinning.
